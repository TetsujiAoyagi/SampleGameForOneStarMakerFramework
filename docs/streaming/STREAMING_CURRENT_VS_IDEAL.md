...
- **現状**: 距離政策は identity と体積で切る。名前から座標を戻すのは、まだ生成器・R-3・factory に残っている。
...
| 生成器の既存収集 / policy | 座標が辞書キー（**M-2**） | identity 文字列 |
| R-3（`SwitchScene` 禁止） | `IsCellId`（名前文法。**M-3**） | 距離政策の候補フラグ |
| 同じ体積に複数 identity | 政策層は許す。生成器の座標キーで潰れる（**M-2**） | 候補集合の差し替えで排他 |
| 格子 | 生成器入力・スポーン・HUD（**M-1 で距離経路から外れた**） | 生成器が体積を焼く入力。HUD 用なら局所 |
| セル | FW の型（`CellIdentity` / `CellScene`。**M-4**） | SampleGame の作業単位 |
| 季節 | FW が知らない（維持） | FW が知らない（維持） |
| LoadType | 親の引っ張り（維持） | 親の引っ張り。値を増やさない |
| ヒステリシス / maxInFlight / 距離順 | 維持 | 維持 |
| 政策 / メカニズム分離 | 維持 | 維持 |

太字の M-* / S-* は作業台（`docs/handoff/`）が持つ移行スライスの番号である。公開面は番号だけを参照し、個別の作業指示には依存しない。
...
M-1 はこの列に入らない。走査範囲を広げたのではなく、**キーを座標から identity ＋ 体積へ替えた**からである。ただし距離経路だけであり、生成器・R-3・factory はまだ座標と名前文法で動いている。
