...
> 本ドキュメントは OneStarMaker 自身の目標宣言である。「何を目標とし、何が強みか」を、実装で確かめられる形で書く。  
...
> **注意:** この観察は「表に出ている事例を全部対照シリーズ化する」宣言ではない。本ドキュメントは候補リストを増やさない。
...
| OutGame 縦糸での寿命契約の実証 | 複雑なゲームロジックを載せた侵食防止の実証（InGame は配線済みだが、載っているのは飛行とセルストリーミングだけ。ゲームルールの複雑さは未投入） |
| 手動 DI で現状足りていること | 手動 DI の恒久性 — **コンストラクタ / Factory が肥大したら VContainer 導入を再評価する合意済み**（03-di.md の再評価条件） |

また、全てを Scene に寄せているわけではない。Camera / Logger / AssetManagement / UICommon は App / Bootstrap 常駐であり、「画面分類木 ≠ 寿命スコープ」を認めて App サービスへ出す判断基準を持つ（[05-scene.md §5.6](../unity/Assets/Docs/Architecture/05-scene.md)）。これは弱点ではなく設計判断だが、「依存と寿命を全部 Scene 構造で表現する」と要約すると過大になる。
...
| 日付 | 内容 |
|---|---|
| 2026-07-25 | 初版。公開事例カタログを削り、深掘りキュー化しない旨を明記 |
