...
> S-3 の矩形集合化は `develop` にマージ済み。機構の記録は [SEASON_LEVELS_IMPLEMENTATION.md](SEASON_LEVELS_IMPLEMENTATION.md)（§1.2〜§1.5 の構図指定は退役）。
...
実装エージェントへ: 本書は構図・実証・スライス順序の正本である。`CellIdentity.TryParse` を修飾対応すること、`StreamingConfig` に qualifier を足すこと、Backend デコレータで id を翻訳することは、**本書の指示ではない。** 距離判断は Bounds Plan が先に決める。
...
4. 生成器を再実行しても、昇格済み identity の演奏レイヤ（`AuthoredRoot` 配下）は 1 個も消えない
...
| 再生成しても編集が残る | 昇格済み identity = `HandAuthored`（`AuthoredRoot` を R-6 が保護）/ それ以外 = `Generated` |
...
| S-9 まで Generated | 各季節の `y=4` 行 | 昇格禁止（S-9 完了まで） | LoadRadius 375m で y=5 フライトに y=4 が必ず入る。源流 `(0,4)(1,4)` の演奏は S-9 のあと |
...
| # | 内容 | 補足 |
|---|---|---|
| Bounds | 現行 4×4 で、距離政策が AABB を読む口を通す | [SCENE_WORLD_BOUNDS.md](SCENE_WORLD_BOUNDS.md)。**通るまで 9×6×4 を焼かない。** 実装分解は別セッション |
| S-4 | 谷の生成と季節スワップ（本体） | Season_* 4 ノード、`World` を置き換え。接頭辞はファイル名。候補集合の差し替え。初期 policy は全 Generated。南辺 4 枚の移送（下表）。**頭で 1 季節 9×6 だけ生成して生成器 1 回の実時間を測り、M を維持するか裁定する** |
...
`set_transform` → 生成器。移送した 4 identity は手編集があるので **昇格して** `HandAuthored` にする
（y=4 の源流 2 枚は S-9 まで昇格を遅らせ、それまでは stamp で生存を見る）。
YAML 手編集・単独 `git mv` を正本にしない。旧 `Generated` 12 枚は破壊経路 3 で削除させる。
...
| W-4 | 編集が消えない | 生成器 2 回のあと、昇格済み stamp 全生存（Environment 増加分含む） |
