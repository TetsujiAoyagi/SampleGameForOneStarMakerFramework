...
- ワールド座標の補正は、移動したセルごとに「旧セル原点 → 新セル原点」の Δ を `AuthoredRoot` に足す
...
| ファイル | 変更 |
|---|---|
| `SampleGame/InGame/InGameSession/Streaming/WorldCellCatalog.cs` | dense 4×4 定数 → **矩形の集合**（矩形リスト + 全セル列挙）。本番は `{ origin=(0,0), size=(4,4) }` の 1 要素。`TryGetCoordinate` は集合の membership。`SpawnPosition` / `CornerSpawn` は現行どおり 4×4 の隅でよい |
...
| `SampleGame/InGame/InGameSession/Streaming/SessionWorldStreamingDriver.cs` | Catalog の全セル列挙を Config へ渡す。S-4 で「今いる季節の矩形」に絞る布石。S-3 は現行 16 セル全部 |
...
| `SampleGame/DependOnAll/Editor/Cells/CellPopulationPlan.cs` | グリッド入力を矩形集合へ一般化。範囲外判定・削除可否・keep 判定は同じ計画経由のまま |
| `OneStarMaker/Tests/` 各所 | `StreamingConfig` の生成箇所を集合 API へ追随 + §3 の新規テスト |

S-3 では触らない: `CellAuthoringPolicy`（南辺 4 枚のまま）、`EnvironmentSproutCells`、`HandEditProbe`、セル `.unity` の中身。これらは S-3C の移送と同時に動かす。
...
2. **破壊経路 3 の再確認から始める**（§33 §12）: `CellPopulationPlanTests` の T-7 / T-11 を読み、矩形集合の範囲外でも同じ保護になることを §3 T-C のレッドテストで先に固定する
3. `StreamingConfig` / `WorldStreamingController` を集合走査へ。既存 WSC テストを単一矩形入力に書き換え、T-B を足す
4. `WorldCellCatalog` / `WorldGridDefinition` / `WorldCellGenerator` / `CellPopulationPlan` を矩形集合へ。本番 Catalog は 4×4 の矩形 1 個
5. 生成器を **1 回** 走らせ、現行 16 セルが削除も増設もされないことを確認する（差分は Generated の焼き込み再出力が出ても、HandAuthored 南辺 4 + Environment 4 が `git status` に出なければよい）
6. `pwsh tools/run-tests.ps1` 全緑（Unity を閉じてから）
...
3. 承認後、Catalog に矩形 4 つを書き、南辺 4 枚を春へ移送（§1.4）、policy / sprout 配列を追随
4. `HandEditProbe.StampHandEdits`（移送前 or 移送直後の生存確認ができるタイミングで）
5. 生成器を実行 → 旧 Generated 12 の削除と新セルの生成 → もう一度実行して HandAuthored 側差分 0
6. `VerifyHandEdits`、テスト全緑、Editor Play で空隙に desired が空であること
7. §33 の D-6 / §5 表 / §7 座標 / O-1 / O-2 を harvest。`pwsh tools/docs-audit.ps1`。本書を `git rm`
