# Game UI Behavior Pipeline - Design Proposal v0.3
...
| v0.3 | 2026-07-09 | 勝利条件の定義、スコープ境界（入力認識・部品間協調・連続追従）、リスクと反証条件（撤退ライン v2）を追加。デザイナーオーサリング経路を将来の拡張へ具体化 |
...
次スライスは「リスクと反証条件（撤退ライン v2）」の検証ユースケースから優先的に選ぶ。得意地形の実績を積み増すのではなく、苦手地形（リスト増減 = レイアウト影響 + 複数要素）で判定することを優先する。

---

## 勝利条件（何をもって「宣言的」の成功とするか）

本設計の「宣言的」は React 型（UI 全体を state の純関数とし、フレームワークが差分適用する）を意味しない。宣言の対象は **Stable State と Behavior 合成のみ** であり、成功条件は以下の 3 点で測定する。

1. 画面固有の遷移中フラグ（`isOpening` 等の if 文）がゼロであること
2. View 側の演出コードが Behavior 構成の宣言のみであること
3. 静止時の表示値が Stable State から一意に導出されること（収束不変条件）

「すべてを UXML / データで記述できること」は勝利条件ではない（原則 4 に従属する将来の拡張である）。

---

## スコープ境界（非目標）

本設計が意図的に担当しない領域を明示する。

### 入力認識（長押し・ジェスチャ）

長押し・ドラッグ・スワイプ等の入力認識は、ステートフルな認識器（UI Toolkit の Manipulator）が担当し、確定した結果のみを Command / Stable State として ViewModel へ流す。Behavior は関与しない。ボタン連打の抑止も Command 層（CanExecute / `IsTransitioning` 購読 / Modal Blocker）で行う。

### 部品間協調（オーケストレーション）

「A の ViewOut 完了を待って B を ViewIn する」等の画面フローは、Scene 層が UniTask で await する命令的コードとして書く。オーケストレーションは本質的に命令的であり、原則 4（コード API を正とする）の下でこれは設計の敗北ではない。

ただし境界の嗅覚として: **Scene 層が遷移中フラグや Visual State を持ち始めたら、それは Runner へ移すべき責務が漏れているサイン**である。汚れてよいのはオーケストレーション（順序の記述）のみとする。

### 連続追従（毎フレーム変化する値）

`Pairwise` による Transition 検出は離散的な状態変化が前提である。毎フレーム変化する値（クールダウン進捗、座標追従等）に対して FromCurrent 割り込みを毎フレーム発生させるのは不適切であり、目標値を追従し続ける常駐型 Behavior（Follow / SmoothDamp 相当）を別プリミティブとして扱う。未実装。次スライス候補。

---

## リスクと反証条件（撤退ライン v2）

T-17 の合格は、フレームワーク設計者自身が選んだ 2 ユースケース（単一要素の遷移 + 割り込み）による判定であり、本設計の**得意地形**での勝利である。判定の信頼性を確保するため、次の撤退ライン判定は苦手地形で行う。

以下のいずれかが観測された場合、Behavior 層の縮小・撤退を再検討する。
