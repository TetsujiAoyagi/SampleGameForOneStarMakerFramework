...
hash 付き bundle を別 worktree / 環境へ渡す場合は、送信元だけでなく受け渡し経路を通したコピーで展開・hash・必須ファイルの閲覧を確認する。Git の改行変換や LFS pointer を内容ファイルと取り違えない。再梱包で変わった bundle の id / hash は更新し、implementation head と区別する。内容を変えない転送修正を、実装の修正や Unity 回帰の再実行理由にしない。
