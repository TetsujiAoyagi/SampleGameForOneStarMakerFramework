...
- 実装開始後に見つかった欠陥は、その失敗が現在の問いを NO-GO にする場合に現スライスで直す。新しい検証項目を受け入れ条件へ追加する場合は、NO-GO 候補であっても Phase A を新しい revision として再開する。その他は所有する後続スライスへ移送する。
...
- 指摘を「現在の問いを阻害する欠陥」と「後続スライスの入力」に分ける。レビュー担当は欠陥を指摘できるが、後者を理由に現スライスの実装や受け入れ条件を自動拡張しない。
...
- 指摘を「現在の問いを阻害する欠陥」と「後続スライスの入力」に分け、後者を理由に現スライスを自動拡張しない。
