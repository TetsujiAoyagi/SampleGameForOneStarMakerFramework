...
- A0 で現況、要求、制約、対象外、未決事項に加え、このスライスが答える問い、次 Phase へ進む最低条件、後続 Phase へ送る問いを同じ planning packet に固定する。
- A1 で主担当が受け入れ条件、責務配置、規模、テスト方針、停止規則を含む初稿を作る。
...
- 実装開始後に現スライスへ受け入れ条件を追加できるのは、その失敗が現在の問いを NO-GO にし、次 Phase を開始させない場合だけとする。その他は所有する後続 Phase へ移送する。
...
- 指摘を「現在の問いを阻害する欠陥」と「後続 Phase の入力」に分ける。レビュー担当は欠陥を指摘できるが、後者を理由に現スライスの実装や受け入れ条件を自動拡張しない。
