...
**このファイルは常時必要な地図と事故防止の契約だけを置く。** 作業手順は末尾の「作業別の追加指示」から共通 Skill を読むこと。
...
- **依存は Game → Framework の一方向。** `Foundation` は Unity パッケージだけ、`Runtime` は `Foundation`、`Debug` は `Foundation` と `Runtime` を参照する。SampleGame の `Common` / `InGame` / `OutGame` 間に循環を作らず、全体配線は `DependOnAll` に集約する。asmdef 参照の追加は設計判断なので勝手に行わない。
- **アセットは `IAssetManagement` 経由。** `AssetOwner`（`App` / `Manual` / `Scene(id)` / `Bind(go)`）で寿命スコープを宣言する。
- **`SceneState` を並べ替えない。** 14 状態の enum 順序は整数比較のガードに使われ、状態変更は `SceneLifecycleManager` が所有する。
...
- **`record` を使わない。** `IsExternalInit` がなく、1つでも使うとプロジェクト全体がコンパイル不能になる。
