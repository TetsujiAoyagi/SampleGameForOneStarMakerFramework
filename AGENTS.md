...
- **ローカルの Unity Editor は、人間が先に開いていなくても作業に必要なら起動・操作してよい。** Phase B の実装担当は、対象を限定した Editor 操作とコンパイル確認まで行える。テストと Build の実行・判定は Phase C の責任とし、必要なら人間の在席を待たずに実行してよい。`unity test` / `unity run` はどの Phase でも使わず、テストは標準の `pwsh tools/run-tests.ps1` を使う。`unity/Library/` と `unity/Temp/UnityLockfile` はブランチを捨てても残るため、起動前に対象projectと既存Editorを確認する。本番の Scene / Prefab / Addressables 等の変更は依頼された範囲に限り、広範な再生成・上書きは個別に影響を確認する。詳細は `osm-unity-editor` Skill に従う。Unity CLIは更新可能、Unity MCPは任意の補助であり作業前提にしない。
