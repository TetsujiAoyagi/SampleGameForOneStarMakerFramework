...
/// <summary>
/// 受信 telemetry export record を NDJSON へ逐次 append し、日次・サイズ上限で rolling する writer。
///
/// <para>
/// enqueue は非ブロッキングで、実際の file I/O は単一 background reader が担う。
/// DebugSocket 受信 callback を disk I/O で止めないため、
/// <see cref="RollingLogFileWriter"/> と同じ Channel + single-reader 設計を踏襲する。
/// </para>
/// </summary>
public sealed class RollingTelemetryFileWriter : IAsyncDisposable
{
    ...
    private readonly Channel<TelemetryExportRecord> _channel;
    private readonly Task _readerTask;
    private int _disposed;

    public RollingTelemetryFileWriter(
        string outputDirectory,
        string baseName = "debugstudio-telemetry",
        ...
        Action<string>? onError = null)
    {
        ...
        _channel = Channel.CreateUnbounded<TelemetryExportRecord>(new UnboundedChannelOptions
        {
            SingleReader = true,
            SingleWriter = false,
        });
    ...
    /// </summary>
    public void Enqueue(TelemetryExportRecord record)
    {
        ...
        {
            ReportError("Telemetry persistence channel is closed; record was dropped.");
        }
    }

    /// <summary>
    /// queue を閉じ、background reader の drain 完了まで待つ。
    /// 終了直前に enqueue された telemetry を失わないため、app lifetime から必ず await する。
    /// </summary>
    public async ValueTask DisposeAsync()
    {
        ...
        {
            ReportError($"Telemetry persistence reader failed during shutdown: {ex}");
        }
    }
        ...
        try
        {
            await foreach (var record in _channel.Reader.ReadAllAsync().ConfigureAwait(false))
            {
                try
                {
                    ...
                    var line = NdjsonTelemetryRecordSerializer.Serialize(record);
                ...
                {
                    ReportError($"Failed to persist telemetry record: {ex}");
                }
            }
        }
        finally
    ...
    private async Task<(FileStream Stream, StreamWriter Writer, long Size, int Sequence)> OpenWriterForTodayAsync(string date)
    {
        ...
        if (existingSize < _maxFileSizeBytes)
        {
            var stream = new FileStream(
                ...
                bufferSize: 4096,
                useAsync: true);
            var writer = new StreamWriter(stream, NdjsonTelemetryRecordSerializer.Utf8WithoutBom);
    ...
    /// <summary>
    /// 保持上限(世代数)を超えた古いファイルを削除する。
    /// 削除失敗で writer loop を止めない。観測欠落よりアプリ停止の方が運用上の被害が大きいため。
    /// </summary>
    private void PruneOldFiles(string activeFilePath)
    {
        try
        {
            ...
            for (var index = 0; index < deleteCount; index++)
            {
                ...
                {
                    ReportError($"Failed to delete old telemetry file '{candidates[index].Path}': {ex}");
                }
            }
        }
        catch (Exception ex)
        {
            ReportError($"Failed to prune old telemetry files: {ex}");
        }
    }
    ...
    private static async Task CloseWriterAsync(StreamWriter? writer, FileStream? stream)
    {
        if (writer != null)
        {
            ...
            {
                Debug.WriteLine($"Failed to flush telemetry writer: {ex}");
            }
    ...
    private void ReportError(string message)
    {
        ...
        {
            Debug.WriteLine($"Telemetry persistence error callback threw: {ex}");
        }
    }
}
